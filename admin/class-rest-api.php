...
final class Rest_Api {
	...
	public function save_widgets( \WP_REST_Request $request ) {
		...
		return rest_ensure_response(
			[
				'success' => true,
				'message' => __( 'Widget settings saved.', 'pixels-elementor-addons' ),
			]
		);
	}
	...
	public function save_extensions( \WP_REST_Request $request ) {
		...
		return rest_ensure_response(
			[
				'success' => true,
				'message' => __( 'Extension settings saved.', 'pixels-elementor-addons' ),
			]
		);
	}
	...
	public function get_form_settings() {
		if ( ! class_exists( '\\PixelsElementorAddons\\Form_Settings' ) ) {
			return new \WP_Error(
				...
				__( 'Form settings require Pixels Addons Pro.', 'pixels-elementor-addons' ),
				[ 'status' => 404 ]
			);
		}

		return rest_ensure_response( self::format_form_settings( \PixelsElementorAddons\Form_Settings::get_all() ) );
	}
	...
	public function save_form_settings( \WP_REST_Request $request ) {
		...
		if ( ! is_array( $params ) ) {
			return new \WP_Error(
				...
				__( 'Invalid form settings payload.', 'pixels-elementor-addons' ),
				[ 'status' => 400 ]
			);
		}
		...
		return rest_ensure_response(
			[
				'success' => true,
				'message' => __( 'Form settings saved.', 'pixels-elementor-addons' ),
			]
		);
	}
	...
	private function persist_form_settings( array $settings ): void {
		...
		\PixelsElementorAddons\Form_Settings::save(
			[
