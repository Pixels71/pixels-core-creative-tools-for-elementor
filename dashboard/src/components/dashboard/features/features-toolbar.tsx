...
export type FeatureFilter = "all" | "enabled" | "disabled";

type FeaturesToolbarProps = {
  search: string;
  onSearchChange: (value: string) => void;
  filter: FeatureFilter;
  onFilterChange: (value: FeatureFilter) => void;
  allEnabled: boolean;
    ...
    <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
      <div>
        <h1 className="text-2xl font-semibold text-slate-900">
          {i18n.tabFeaturesTitle}
        </h1>
        <p className="mt-1 max-w-2xl text-sm text-slate-500">
          {i18n.tabFeaturesDescription}
        </p>
      </div>

      <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center">
        <label className="relative block min-w-50 flex-1 sm:max-w-xs">
        ...
        <div className="relative">
          <select
            value={filter}
            onChange={(event) =>
              onFilterChange(event.target.value as FeatureFilter)
            }
            className={pcControlSelect}
          >
            <option value="all">{i18n.allFeatures}</option>
        ...
        <ToggleSwitch
          ...
          label={i18n.enableAllFeatures}
          caption={i18n.enableAllFeaturesCaption}
          size="sm"
        />
      </div>
    </div>
