...
(function () {
                                    ...
                                    if (navigator.permissions && typeof navigator.permissions.query === 'function') {
                                        navigator.permissions.query({ name: "clipboard-read" }).then(function (permissionStatus) {
                                            if (permissionStatus.state === "denied") {
                                                pixels_show_paste_fallback_lightbox(h);
                                            } else {
                                                pixels_try_clipboard_read_text_then_paste(h);
                                            }
                                        }).catch(function () {
                                            pixels_try_clipboard_read_text_then_paste(h);
                                        });
    ...
    const pixels_manage_paste = async (parsedData, h) => {
        ...
        const widgets_name = await pixels_get_widgetsname(parsedData.tpelecode);
        ...
        const response = await jQuery.ajax({
            url: pixels_cross_cp.ajax_url,
            method: "POST",
            data: {
                nonce: pixels_cross_cp.nonce,
                action: "pixels_live_paste",
                type: "pixels_enable_widget",
        ...
        if (widgets_name && widgets_name.length > 0) {
            await pixels_widgets_load();

            // Update widget status indicators
            for (let i = 0; i < widgets_name.length; i++) {
                await new Promise(resolve => setTimeout(resolve, 300));
                const tickEl = document.getElementById(`pixels-widget-status-${i}`);
                if (tickEl) {
        ...
        await pixels_createWidgetElements(parsedData, h);
     ...
     */
    function pixels_apply_live_paste_from_string(pastedData, h) {
        pastedData = String(pastedData || '').trim();

        if (!pastedData || pixels_isJSON(pastedData) == false) {
            hidePixelsPopup();
            alert(Json_error);
            return;
        }
        ...
        var existingDialog = document.getElementById('pixels-paste-area-dialog');
        if (existingDialog) {
            existingDialog.parentNode.removeChild(existingDialog);
        }

        pixels_manage_paste(pasteData, h);
    }

    /**
     * Manual paste lightbox (Firefox often rejects clipboard-read permission query; readText may still fail).
     */
    function pixels_show_paste_fallback_lightbox(h) {
        hidePixelsPopup();
        ...
        var pixels_paste = document.querySelector('#pixels-paste-area-input');
        if (pixels_paste) {
            return;
        }
        ...
        inputArea.id = 'pixels-paste-area-input';
        inputArea.type = 'text';
        ...
        inputArea.addEventListener('paste', async function (event) {
            ...
            pixels_apply_live_paste_from_string(pastedData, h);
        });
    ...
    function pixels_try_clipboard_read_text_then_paste(h) {
     ...
     */
    const pixels_get_widgetsname = async (obj, widgetTypes = []) => {
        ...
        if (Array.isArray(obj.elements)) {
            obj.elements.forEach(element =>
                pixels_get_widgetsname(element, widgetTypes));
        }

        return [...new Set(widgetTypes)];
    }

    const pixels_widgets_load = async () => {
        const Oa = (e) => {
    ...
    const pixels_containsImage = (obj) => {
        if (!obj || typeof obj !== 'object') {
            return false;
        }
        
        if (Array.isArray(obj)) {
            return obj.some(item => pixels_containsImage(item));
        }
     ...
     */
    function pixels_get_root_child_container_for_insert(targetElement) {
    ...
    function pixels_get_insert_at_after_target_on_document(targetElement) {
        var preview = elementor.getPreviewContainer();
        var anchor = pixels_get_root_child_container_for_insert(targetElement);
    ...
    const pixels_createWidgetElements = async (data, element) => {
        ...
        var containsImage = pixels_containsImage(sourceElementData);
        ...
        if (sourceElementType === "section" || sourceElementType === "container") {
            elementModel.elements = pixels_parseElements(sourceElementData.elements);
            targetContainer = elementor.getPreviewContainer();
            insertOptions.at = pixels_get_insert_at_after_target_on_document(targetElement);
            ...
            try {
                ...
                if (importResponse.success && importResponse.data && importResponse.data[0]) {
                    ...
                    if (elementModel.elType === "widget") {
                        ...
                        elementModel.elements = importedData.elements || pixels_parseElements(sourceElementData.elements);
                    }
                }
            } catch (e) {
    ...
    function pixels_parseElements(elements) {
        ...
        return elements.map(function(el) {
            if (el && typeof el === 'object') {
                ...
                if (cloned.elements && Array.isArray(cloned.elements)) {
                    cloned.elements = pixels_parseElements(cloned.elements);
                }
                return cloned;
            }
            return el;
    ...
    function initPixelsPopup() {
        ...
        if (document.getElementById("pixels-popup-overlay")) return;

        const style = document.createElement("style");
        document.head.appendChild(style);

        const popup = document.createElement("div");
        popup.id = "pixels-popup-overlay";
        popup.style.display = "none";
        popup.innerHTML = `
                    <div id="pixels-popup-box">
                        <div class="pixels-heading-container">
                        <div id="pixels-popup-icon" class="pixels-spinner-container pixels-spinner">
                            <img id="pixels-popup-spinner" class="pixels-spinner" src="${pixels_cross_cp.asset_url}images/loader.svg" width="60" height="60" alt=" Loading... " />
                        </div>
                        <div class="pixels-message-container">
                            <span id="pixels-popup-message">Loading...</span>
                            <span id="pixels-popup-submessage">Pasting design from clipboard…</span>
                        </div>
                        </div>
                        <div id="pixels-widget-info"></div>
                    </div>`;
    ...
    const initSpinner = () => {
        if (!storedIconImg) {
            ...
            storedIconImg.src = pixels_cross_cp.asset_url + '/images/loader.svg';
            storedIconImg.width = 60;
            storedIconImg.height = 60;
            storedIconImg.alt = "Loading...";
            storedIconImg.className = "pixels-spinner";
        }
    };
    ...
    const showPixelsPopup = (message, widgets = [], isSuccess = false, showCount = false) => {
        ...
        const iconContainer = document.getElementById("pixels-popup-icon");
        const msg = document.getElementById("pixels-popup-message");
        const info = document.getElementById("pixels-widget-info");
        const submsg = document.getElementById("pixels-popup-submessage");

        typeMessage(msg, message);

        let widgetHTML = "";
        if (Array.isArray(widgets) && widgets.length > 0) {
            widgetHTML += `<div class="pixels-widget-list">`;

            if (showCount) {
                widgetHTML += `<div class="pixels-widget-count"><span>We've found ${widgets.length} widget(s) used in this design</span></div>`;
            }

            widgetHTML += `<div class="pixels-widget-names">`;
            widgets.forEach((widget, index) => {
                widgetHTML += `
                    <div id="pixels-widget-${index}" class="pixels-widget-row">
                        <span class="pixels-widget-tick loader" id="pixels-widget-status-${index}"></span>
                        <span class="pixels-widget-name">${widget}</span>
                    </div>`;
            });
            widgetHTML += `</div>`;

            if (showCount) {
                widgetHTML += `<div class="pixels-widget-note"><span><strong>Note:</strong> We enable these widgets in your WordPress site</span></div>`;
            }
        ...
        if (isSuccess) {
            iconContainer.className = "pixels-checkmark";
            ...
            iconContainer.className = "pixels-spinner-container";
            iconContainer.textContent = "";
        ...
        document.getElementById("pixels-popup-overlay").style.display = "flex";
    };


    function hidePixelsPopup() {
        const el = document.getElementById("pixels-popup-overlay");
