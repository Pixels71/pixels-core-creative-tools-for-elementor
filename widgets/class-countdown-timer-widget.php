...
class Countdown_Timer_Widget extends Widget_Base {
	...
	public function get_keywords(): array {
		return array( 'countdown', 'timer', 'pixeccte' );
	}
	...
	/**
	 * Register individual unit item style controls.
	 *
	...
	private function register_individual_unit_item_style_controls( string $unit, string $label ): void {
		...
		$visibility = array( 'show_' . $unit => 'yes' );
	...
	/**
	 * Register individual unit digit style controls.
	 *
	 * @param string $unit Unit.
	 * @param string $label Label.
	 */
	...
	/**
	 * Register individual unit label style controls.
	 *
	 * @param string $unit Unit.
	 * @param string $label Label.
	 */
	...
	private function prepare_timer_data( array $settings ): ?array {
		...
		if ( empty( $active_units ) ) {
			$active_units = array( 'days', 'hours', 'minutes', 'seconds' );
		}
	...
	protected function render(): void {
		...
		if ( $show_circle_progress ) {
			...
			if ( in_array( $ring_label_position, array( 'top', 'bottom' ), true ) ) {
		...
		if ( $show_separator && in_array( $separator_style, array( 'colon', 'line' ), true ) ) {
