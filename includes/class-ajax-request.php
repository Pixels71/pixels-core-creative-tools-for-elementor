...
class Pixels_Ajax_Request {
	...
	public function pixels_live_paste() {
		if ( ! check_ajax_referer( 'pixels_cross_cp_import', 'nonce', false ) ) {
			wp_send_json(
				$this->pixels_set_response(
					false,
					__( 'Invalid nonce.', 'pixels-elementor-addons' ),
					__( 'The security check failed. Please refresh the page and try again.', 'pixels-elementor-addons' )
				)
			);
		}
		...
		if ( ! current_user_can( 'edit_posts' ) ) {
			wp_send_json(
				$this->pixels_set_response(
					false,
					__( 'Permission denied.', 'pixels-elementor-addons' ),
					__( 'You do not have permission to perform this action.', 'pixels-elementor-addons' )
				)
			);
		}
		...
		switch ( $type ) {
			...
			default:
				$response = $this->pixels_set_response(
					false,
					__( 'Invalid type.', 'pixels-elementor-addons' ),
					__( 'The action type is not valid.', 'pixels-elementor-addons' )
				);
				break;
		}
	...
	public function pixels_enable_widget( $widgets_name = [] ) {
		if ( ! current_user_can( 'manage_options' ) ) {
			return $this->pixels_set_response(
				false,
				__( 'Permission denied.', 'pixels-elementor-addons' ),
				__( 'Only administrators can enable widgets from Live Paste.', 'pixels-elementor-addons' )
			);
		}
		...
		return $this->pixels_set_response( true, __( 'Widgets enabled successfully.', 'pixels-elementor-addons' ), '' );
	}
	...
	public function pixels_cross_copy_paste_media_import() {
		...
		if ( ! current_user_can( 'edit_posts' ) || ! current_user_can( 'upload_files' ) ) {
			wp_send_json_error( __( 'You do not have permission to import media.', 'pixels-elementor-addons' ), 403 );
		}

		if ( ! isset( $_POST['copy_content'] ) ) {
			wp_send_json_error( __( 'Empty Content.', 'pixels-elementor-addons' ) );
		}
		...
		if ( ! is_array( $decoded ) ) {
			wp_send_json_error( __( 'Invalid Content.', 'pixels-elementor-addons' ) );
		}
