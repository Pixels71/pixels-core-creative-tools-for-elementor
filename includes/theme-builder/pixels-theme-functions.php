 ...
 * @return string
 */
function pixels_get_theme_template_type_meta_key() {
	return 'pixels_hf_template_type';
}

/**
 * Resolve a pixels-theme post ID for template-type helpers.
 *
 * @param int $post_id Optional template post ID.
 * @return int
 */
function pixels_resolve_theme_template_post_id( $post_id = 0 ) {
...
function pixels_get_theme_template_type( $post_id = 0 ) {
	$post_id = pixels_resolve_theme_template_post_id( $post_id );

	if ( ! $post_id || 'pixels-theme' !== get_post_type( $post_id ) ) {
		return '';
	}

	$meta_key = pixels_get_theme_template_type_meta_key();
 ...
 * @return void
 */
function pixels_update_theme_template_type( $post_id, $type ) {
	$post_id = absint( $post_id );
	...
	update_post_meta( $post_id, pixels_get_theme_template_type_meta_key(), (string) $type );
	delete_post_meta( $post_id, 'ehf_template_type' );
}

/**
 * One-time migration from ehf_template_type to pixels_hf_template_type.
 *
 * @return void
 */
function pixels_maybe_migrate_theme_template_type_meta() {
	...
	foreach ( $post_ids as $post_id ) {
		...
		if ( '' === (string) get_post_meta( $post_id, pixels_get_theme_template_type_meta_key(), true ) ) {
			update_post_meta( $post_id, pixels_get_theme_template_type_meta_key(), $legacy );
		}
