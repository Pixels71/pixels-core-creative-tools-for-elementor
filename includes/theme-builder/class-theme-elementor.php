...
class Theme_Elementor {
	...
	public function __construct() {
		...
		if ( defined( 'ELEMENTOR_VERSION' ) && is_callable( 'Elementor\Plugin::instance' ) ) :
			...
			add_action( 'init', array( $this, 'setup_unsupported_theme' ) );
			add_action( 'elementor/documents/register_controls', array( $this, 'register_popup_document_controls' ) );
			add_action( 'elementor/documents/register_controls', array( $this, 'register_mega_menu_document_controls' ) );
			add_action( 'elementor/documents/register_controls', array( $this, 'register_single_post_preview_controls' ) );
			add_filter( 'elementor/document/save/data', array( $this, 'preserve_popup_position_settings_on_save' ), 10, 2 );
			add_filter( 'elementor/document/save/data', array( $this, 'save_template_preview_post_on_document_save' ), 10, 2 );

			// Scripts and styles.
			add_action( 'wp_enqueue_scripts', array( $this, 'enqueue_scripts' ) );
			add_action( 'elementor/editor/before_enqueue_scripts', array( $this, 'enqueue_popup_editor_live_preview' ) );
			add_action( 'elementor/editor/before_enqueue_scripts', array( $this, 'enqueue_single_post_preview_editor_script' ) );

			add_action( 'admin_enqueue_scripts', array( $this, 'enqueue_admin_scripts' ) );

			add_filter( 'body_class', array( $this, 'body_class' ) );
			add_action( 'switch_theme', array( $this, 'reset_unsupported_theme_notice' ) );
			add_action( 'wp_footer', array( $this, 'render_popups' ), 40 );
			add_action( 'elementor/page_templates/canvas/before_content', array( $this, 'open_popup_editor_wrapper' ), 1 );
			add_action( 'elementor/page_templates/canvas/after_content', array( $this, 'close_popup_editor_wrapper' ), 99 );

			add_shortcode( 'pixeccte_hf_template', array( $this, 'render_template' ) );
			add_filter( 'nav_menu_css_class', array( $this, 'add_mega_menu_item_classes' ), 10, 4 );
			add_filter( 'nav_menu_link_attributes', array( $this, 'add_mega_menu_link_attributes' ), 10, 4 );
			add_filter( 'walker_nav_menu_start_el', array( $this, 'render_mega_menu_panel' ), 10, 4 );

			// Use Elementor theme builder templates as full page templates when available.
			add_filter( 'template_include', array( $this, 'maybe_use_popup_canvas_template' ), 12 );
			add_filter( 'template_include', array( $this, 'maybe_use_404_template' ), 19 );
			add_filter( 'template_include', array( $this, 'maybe_use_archive_post_template' ), 20 );
			add_filter( 'template_include', array( $this, 'maybe_use_single_post_template' ), 21 );
	...
	public function includes() {
		...
		require_once PIXECCTE_PATH . 'includes/theme-builder/class-admin.php';

		// Load Target rules.
		require_once PIXECCTE_PATH . 'includes/theme-builder/class-target-rules-fields.php';
		// Setup upgrade routines.
		require_once PIXECCTE_PATH . 'includes/theme-builder/class-update.php';
	}
	...
	public function register_popup_document_controls( $document ) {
		...
		$document->add_control(
			...
			array(
				...
				'default'    => array(
					'size' => $options['height'] ? (int) $options['height'] : 500,
					'unit' => 'px',
				),
				'condition'  => array( 'pixeccte_popup_height_type' => 'custom' ),
				'selectors'  => array(
				...
				$document->add_control(
					...
					array_merge(
						...
						array(
							...
							'condition'   => array( 'pixeccte_popup_trigger_' . $key => 'yes' ),
							'render_type' => 'template',
						)
					)
				);
		...
		$document->add_control(
			...
			array(
				...
				'condition'   => array( 'pixeccte_popup_show_overlay' => 'yes' ),
				'selectors'   => array(
		...
		foreach ( array(
			'entrance' => esc_html__( 'Entrance Animation', 'pixels-core-creative-tools-for-elementor' ),
			'exit'     => esc_html__( 'Exit Animation', 'pixels-core-creative-tools-for-elementor' ),
		) as $key => $label ) {
			$document->add_control(
		...
		$document->add_control(
			...
			array(
				...
				'condition'   => array( 'pixeccte_popup_show_close_button' => 'yes' ),
				'render_type' => 'template',
			)
		);
			...
			$document->add_control(
				...
				array(
					...
					'range'       => array(
						'px' => array(
							'min' => 0,
							'max' => in_array( $key, array( 'close_button_offset_x', 'close_button_offset_y' ), true ) ? 100 : 200,
						),
					),
	...
	/**
	 * Register mega menu document controls.
	 *
	 * @param mixed $document Document.
	 */
	...
	/**
	 * Preserve popup position settings on save.
	 *
	...
	public function enqueue_scripts() {
		wp_enqueue_style( 'pixeccte-theme-style', PIXECCTE_URL . 'assets/css/pixeccte-header-footer.css', array(), PIXECCTE_VERSION );
		wp_enqueue_script( 'pixeccte-mega-menu', PIXECCTE_URL . 'assets/js/pixeccte-mega-menu.js', array(), PIXECCTE_VERSION, true );
		...
		if ( is_singular( 'pixeccte-theme' ) && 'type_popup' === pixeccte_get_theme_template_type( get_the_ID() ) ) :
			wp_enqueue_style( 'pixeccte-theme-popup', PIXECCTE_URL . 'assets/theme-builder/css/pixeccte-theme-popup.css', array(), PIXECCTE_VERSION );
		endif;
		...
		if ( ! empty( $popup_ids ) ) :
			wp_enqueue_style( 'pixeccte-theme-popup', PIXECCTE_URL . 'assets/theme-builder/css/pixeccte-theme-popup.css', array(), PIXECCTE_VERSION );
			wp_enqueue_script( 'pixeccte-theme-popup', PIXECCTE_URL . 'assets/theme-builder/js/pixeccte-theme-popup.js', array(), PIXECCTE_VERSION, true );
	...
	public function enqueue_admin_scripts() {
		...
		if ( ( 'pixeccte-theme' === $screen->id && ( 'post.php' === $pagenow || 'post-new.php' === $pagenow ) ) || ( 'edit.php' === $pagenow && 'edit-pixeccte-theme' === $screen->id ) ) :
			wp_enqueue_style( 'pixeccte-theme-admin-style', PIXECCTE_URL . 'assets/theme-builder/css/pixeccte-theme-admin.css', array(), PIXECCTE_VERSION );
			wp_enqueue_script( 'pixeccte-theme-admin-script', PIXECCTE_URL . 'assets/theme-builder/js/pixeccte-theme-admin.js', array(), PIXECCTE_VERSION, true );
		endif;
	...
	/**
	 * Enqueue popup editor live preview.
	 */
	...
	public function setup_unsupported_theme() {
		if ( ! current_theme_supports( 'pixeccte' ) ) :
			require_once PIXECCTE_PATH . 'includes/theme-builder/themes/default/class-default-compat.php';
		endif;
	}
	...
	private function get_mega_menu_options( $template_id ) {
		...
		if ( ! in_array( $width_type, array( 'custom', 'container', 'full_width' ), true ) ) :
			$width_type = 'custom';
		endif;
	 ...
	 * @param array  $default_value Default slider value.
	 * @param array  $units         Allowed units.
	 * @param array  $ranges        Unit ranges.
	 *
	 * @return array
	 */
	private function normalize_mega_menu_responsive_width( $settings, $key, $default_value, $units, $ranges ) {
		return array(
			'desktop' => $this->normalize_mega_menu_width_value(
				isset( $settings[ $key ] ) ? $settings[ $key ] : array(),
				$default_value,
				$units,
				$ranges
			),
			'tablet'  => $this->normalize_mega_menu_width_value(
				isset( $settings[ $key . '_tablet' ] ) ? $settings[ $key . '_tablet' ] : array(),
				$default_value,
				$units,
				$ranges
			),
			'mobile'  => $this->normalize_mega_menu_width_value(
				isset( $settings[ $key . '_mobile' ] ) ? $settings[ $key . '_mobile' ] : array(),
				$default_value,
				$units,
				$ranges
			),
	 ...
	 * @return array
	 */
	private function normalize_mega_menu_width_value( $width, $default_value, $units, $ranges ) {
		if ( ! is_array( $width ) || ! isset( $width['size'], $width['unit'] ) ) :
			return $default_value;
		endif;

		$unit  = in_array( $width['unit'], $units, true ) ? $width['unit'] : $default_value['unit'];
		$size  = is_numeric( $width['size'] ) ? (float) $width['size'] : $default_value['size'];
		$range = isset( $ranges[ $unit ] ) ? $ranges[ $unit ] : array( 0, 2000 );
	 ...
	 * @param mixed    $args        Nav menu args (unused; required by walker filter).
	 *
	 * @return string
	 */
	public function render_mega_menu_panel( $item_output, $item, $depth = 0, $args = null ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed -- Walker filter signature.
		unset( $args );
		$template_id = $this->get_mega_menu_template_id_from_item( $item );
	 ...
	 * @param  mixed $default_value Default value to be received if the option value is not stored in the option.
	 *
	 * @return mixed.
	 */
	public static function get_settings( $setting = '', $default_value = '' ) {
	...
	public function render_popup_editor_template() {
		...
		<div id="pixeccte-theme-popup-editor-preview-<?php echo esc_attr( (string) $popup_id ); ?>" class="<?php echo esc_attr( implode( ' ', $classes ) ); ?>" aria-hidden="false">
			...
			<div class="pixeccte-theme-popup__dialog" role="dialog" aria-modal="true" tabindex="-1" style="<?php echo esc_attr( $dialog_style ); ?>">
				...
				<div class="pixeccte-theme-popup__content" style="<?php echo esc_attr( $content_style ); ?>">
					<?php
					$module = apply_filters( 'elementor/render_mode/module', 'page-templates' ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound,WordPress.NamingConventions.ValidHookName.UseUnderscores -- Elementor core hook.
	...
	private function get_popup_options( $popup_id ) {
		...
		$options       = wp_parse_args( is_array( $options ) ? $options : array(), $this->get_default_popup_options() );
		...
		$meta_trigger       = get_post_meta( $popup_id, 'pixeccte_popup_trigger', true );
		$options['trigger'] = ! empty( $page_settings['pixeccte_popup_trigger'] )
			? sanitize_key( $page_settings['pixeccte_popup_trigger'] )
			: ( ! empty( $meta_trigger ) ? $meta_trigger : $options['trigger'] );
		...
		foreach ( array(
			...
			if ( array_key_exists( $setting_key, $page_settings ) && '' !== $page_settings[ $setting_key ] ) {
				...
				if ( in_array( $key, array( 'scroll_to_selector', 'class_click_selector', 'open_selector' ), true ) ) {
					$options[ $key ] = $this->sanitize_popup_selector( $value );
					continue;
				}

				if ( 'css_classes' === $key ) {
					$class_parts     = preg_split( '/\s+/', $value );
					$options[ $key ] = implode(
						' ',
						array_filter( array_map( 'sanitize_html_class', false !== $class_parts ? $class_parts : array() ) )
					);
					continue;
				}
		...
		foreach ( array(
			'scroll_percent'   => array( 0, 100 ),
			'click_count'      => array( 1, 100 ),
			'inactivity_delay' => array( 0, 3600 ),
			'adblock_delay'    => array( 0, 3600 ),
			'page_load_delay'  => array( 0, 3600 ),
		) as $key => $range ) {
			$setting_key = 'pixeccte_popup_' . $key;
		...
		$options['scroll_direction']                 = in_array( $options['scroll_direction'], array( 'down', 'up' ), true ) ? $options['scroll_direction'] : 'down';
		$options['close_button_placement']           = in_array( $options['close_button_placement'], array( 'inside', 'outside' ), true ) ? $options['close_button_placement'] : 'inside';
		$options['close_button_horizontal_position'] = in_array( $options['close_button_horizontal_position'], array( 'left', 'right' ), true ) ? $options['close_button_horizontal_position'] : 'right';
		$options['close_button_vertical_position']   = in_array( $options['close_button_vertical_position'], array( 'top', 'bottom' ), true ) ? $options['close_button_vertical_position'] : 'top';
		$options['content_border_style']             = in_array( $options['content_border_style'], array( 'none', 'solid', 'dashed', 'dotted', 'double' ), true ) ? $options['content_border_style'] : 'dashed';

		foreach ( array( 'close_button_offset_x', 'close_button_offset_y', 'close_button_size', 'close_button_icon_size', 'close_button_border_width', 'close_button_border_radius', 'content_border_width', 'content_border_radius' ) as $key ) {
			$setting_key = 'pixeccte_popup_' . $key;
		...
		foreach ( array( 'show_overlay', 'show_close_button', 'prevent_overlay_close', 'prevent_esc_close', 'disable_scroll', 'avoid_multiple', 'accessible_navigation' ) as $key ) {
			$setting_key = 'pixeccte_popup_' . $key;
		...
		$options['margin']  = wp_parse_args(
			is_array( $options['margin'] ) ? $options['margin'] : array(),
			array(
				'top'    => 0,
				'right'  => 0,
				'bottom' => 0,
		...
		$options['padding'] = wp_parse_args(
			is_array( $options['padding'] ) ? $options['padding'] : array(),
			array(
				'top'    => 0,
				'right'  => 0,
				'bottom' => 0,
	...
	private function enqueue_popup_responsive_css( $selector, $popup_id, $options ) {
		...
		wp_register_style( $handle, false, array(), PIXECCTE_VERSION );
		wp_enqueue_style( $handle );
		wp_add_inline_style( $handle, $css );
	}

	/**
	 * Popup get responsive position.
	 *
	 * @param mixed $settings Settings.
	 * @param mixed $key Key.
	 * @param mixed $axis Axis.
	 */
	...
	/**
	 * Popup dimensions to spacing.
	 *
	 * @param mixed $dimensions Dimensions.
	 */
	...
	/**
	 * Popup dimension to css.
	 *
	 * @param mixed $dimension Dimension.
	...
	/**
	 * Popup box shadow to css.
	 *
	 * @param mixed $shadow Shadow.
	...
	private function popup_box_shadow_to_css( $shadow, $type ) {
		...
		return $position . implode( ' ', array( $horizontal, $vertical, $blur, $spread, $color ) );
	}

	/**
	 * Popup box shadow length to css.
	 *
	...
	/**
	 * Popup css size to dimension.
	 *
	 * @param mixed $value Value.
	 * @param mixed $default_size Default size.
	 * @param mixed $default_unit Default unit.
	 */
	...
	/**
	 * Popup horizontal position to flex.
	 *
	 * @param mixed $value Value.
	 */
	...
	/**
	 * Popup vertical position to flex.
	 *
	 * @param mixed $value Value.
	 */
	...
	/**
	 * Popup flex to horizontal position.
	 *
	 * @param mixed $value Value.
	 */
	...
	/**
	 * Popup flex to vertical position.
	 *
	 * @param mixed $value Value.
	 */
	...
	/**
	 * Get popup close button style.
	 *
	 * @param mixed $options Options.
	 */
	private function get_popup_close_button_style( $options ) {
		$horizontal = in_array( $options['close_button_horizontal_position'], array( 'left', 'right' ), true ) ? $options['close_button_horizontal_position'] : 'right';
		$vertical   = in_array( $options['close_button_vertical_position'], array( 'top', 'bottom' ), true ) ? $options['close_button_vertical_position'] : 'top';
	...
	private function sanitize_popup_border_style( $value, $fallback ) {
		...
		return in_array( $value, array( 'none', 'solid', 'dashed', 'dotted', 'double' ), true ) ? $value : $fallback;
	}

	/**
	 * Sanitize popup box shadow.
	 *
	 * @param mixed $value Value.
	 * @param mixed $fallback Fallback.
	 */
	...
	public function render_template( $atts ) {
		...
		if ( ! in_array( $post->post_status, array( 'publish' ), true ) && ! current_user_can( 'edit_post', $id ) ) {
			return '';
		}
