...
class Target_Rules_Fields {
	...
	public static function get_location_selections() {
		...
		$special_pages = array(
			'special-404'    => __( '404 Page', 'pixels-elementor-addons' ),
			'special-search' => __( 'Search Page', 'pixels-elementor-addons' ),
			'special-blog'   => __( 'Blog / Posts Page', 'pixels-elementor-addons' ),
			'special-front'  => __( 'Front Page', 'pixels-elementor-addons' ),
			'special-date'   => __( 'Date Archive', 'pixels-elementor-addons' ),
			'special-author' => __( 'Author Archive', 'pixels-elementor-addons' )
		);

		if ( class_exists( 'WooCommerce' ) ) :
			$special_pages['special-woo-shop'] = __( 'WooCommerce Shop Page', 'pixels-elementor-addons' );
		endif;

		$selection_options = array(
			'basic'         => array(
				'label' => __( 'Basic', 'pixels-elementor-addons' ),
				'value' => array(
					'basic-global'    => __( 'Entire Website', 'pixels-elementor-addons' ),
					'basic-singulars' => __( 'All Singulars', 'pixels-elementor-addons' ),
					'basic-archives'  => __( 'All Archives', 'pixels-elementor-addons' )
				)
			),

			'special-pages' => array(
				'label' => __( 'Special Pages', 'pixels-elementor-addons' ),
				'value' => $special_pages
			)
		);
		...
		$selection_options['specific-target'] = array(
			'label' => __( 'Specific Target', 'pixels-elementor-addons' ),
			'value' => array(
				'specifics' => __( 'Specific Pages / Posts / Taxonomies, etc.', 'pixels-elementor-addons' )
			)
		);
	...
	function get_posts_by_query() {
		...
		if ( ! current_user_can( 'edit_posts' ) ) {
			wp_send_json_error( __( 'You do not have permission to search posts.', 'pixels-elementor-addons' ), 403 );
		}
	...
	public function admin_styles() {
		 ...
		 */
		$localize_vars = array(
			'please_enter'  => __( 'Please enter', 'pixels-elementor-addons' ),
			'please_delete' => __( 'Please delete', 'pixels-elementor-addons' ),
			'more_char'     => __( 'or more characters', 'pixels-elementor-addons' ),
			'character'     => __( 'character', 'pixels-elementor-addons' ),
			'loading'       => __( 'Loading more results…', 'pixels-elementor-addons' ),
			'only_select'   => __( 'You can only select', 'pixels-elementor-addons' ),
			'item'          => __( 'item', 'pixels-elementor-addons' ),
			'char_s'        => __( 's', 'pixels-elementor-addons' ),
			'no_result'     => __( 'No results found', 'pixels-elementor-addons' ),
			'searching'     => __( 'Searching…', 'pixels-elementor-addons' ),
			'not_loader'    => __( 'The results could not be loaded.', 'pixels-elementor-addons' ),
			'search'        => __( 'Search pages / post / categories', 'pixels-elementor-addons' ),
	...
	public static function target_rule_settings_field( $name, $settings, $value ) {
		...
		$add_rule_label = isset( $settings['add_rule_label'] ) ? $settings['add_rule_label'] : __( 'Add Rule', 'pixels-elementor-addons' );
		$saved_values   = $value;
		$output         = '';
		...
		$output .= '<option value="">' . __( 'Select', 'pixels-elementor-addons' ) . '</option>';
		...
		/* Wrapper Start */
		$output .= '<div class="pixels-target-rule-wrapper pixels-target-rule-' . $rule_type . '-on-wrap" data-type="' . $rule_type . '">';
		$output .= '<div class="pixels-target-rule-selector-wrapper pixels-target-rule-' . $rule_type . '-on">';
		...
		// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- $output safely built with escaped variables
		echo $output;
	}
	...
	public static function get_post_target_rule_options( $post_type, $taxonomy ) {
		...
		$all_posts                          = sprintf( __( 'All %s', 'pixels-elementor-addons' ), $post_label );
		$post_option[ $post_name . '|all' ] = $all_posts;

		if ( 'pages' != $post_key ) :
			/* translators: %s post label */
			$all_archive                                = sprintf( __( 'All %s Archive', 'pixels-elementor-addons' ), $post_label );
		...
		if ( in_array( $post_type->name, $taxonomy->object_type ) ) :
			...
			$tax_archive = sprintf( __( 'All %s Archive', 'pixels-elementor-addons' ), $tax_label );
	...
	public static function generate_target_rule_selector( $type, $selection_options, $input_name, $saved_values, $add_rule_label ) {
		...
		foreach ( $saved_values['rule'] as $index => $data ) :
			$output .= '<div class="pixels-target-rule-condition pixels-target-rule-' . $index . '" data-rule="' . $index . '" >';
			/* Condition Selection */
			$output .= '<span class="target_rule-condition-delete dashicons dashicons-dismiss"></span>';
			$output .= '<div class="target_rule-condition-wrap" >';
			$output .= '<select name="' . esc_attr( $input_name ) . '[rule][' . $index . ']" class="target_rule-condition form-control pixels-input">';
			$output .= '<option value="">' . __( 'Select', 'pixels-elementor-addons' ) . '</option>';
		...
		if ( 'display' == $type ) :
			...
			$output .= '<a href="#" class="button">' . __( 'Add Exclusion Rule', 'pixels-elementor-addons' ) . '</a>';
			$output .= '</div>';
		endif;
