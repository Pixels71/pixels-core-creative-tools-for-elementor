 ...
 */
namespace PixelsElementorAddons\Theme_Builder;

use PixelsElementorAddons\Theme_Builder\Target_Rules_Fields;
...
class Theme_Elementor {
	...
	public function register_popup_document_controls( $document ) {
		...
		if ( ! $post || 'pixels-theme' !== $post->post_type || 'type_popup' !== get_post_meta( $post->ID, 'ehf_template_type', true ) ) {
			return;
		}
		...
		$document->start_controls_section(
			...
			[
				'label' => esc_html__( 'Pixels Popup Layout', 'pixels-elementor-addons' ),
		...
		$document->add_control(
			...
			[
				...
				'options' => [
					'fit'    => esc_html__( 'Fit To Content', 'pixels-elementor-addons' ),
					'custom' => esc_html__( 'Custom', 'pixels-elementor-addons' ),
				],
		...
		$document->add_control(
			'pixels_popup_height',
			[
				'label'      => esc_html__( 'Custom Height', 'pixels-elementor-addons' ),
				'type'       => \Elementor\Controls_Manager::SLIDER,
				'size_units' => [ 'px', 'vh' ],
				'range'      => [
					'px' => [ 'min' => 100, 'max' => 2000 ],
		...
		$document->add_responsive_control(
			...
			[
				...
				'options' => [
					'left'   => [ 'title' => esc_html__( 'Left', 'pixels-elementor-addons' ), 'icon' => 'eicon-h-align-left' ],
					'center' => [ 'title' => esc_html__( 'Center', 'pixels-elementor-addons' ), 'icon' => 'eicon-h-align-center' ],
					'right'  => [ 'title' => esc_html__( 'Right', 'pixels-elementor-addons' ), 'icon' => 'eicon-h-align-right' ],
				],
				'render_type' => 'template',
			]
		);
		...
		$document->add_responsive_control(
			...
			[
				...
				'options' => [
					'top'    => [ 'title' => esc_html__( 'Top', 'pixels-elementor-addons' ), 'icon' => 'eicon-v-align-top' ],
					'center' => [ 'title' => esc_html__( 'Center', 'pixels-elementor-addons' ), 'icon' => 'eicon-v-align-middle' ],
					'bottom' => [ 'title' => esc_html__( 'Bottom', 'pixels-elementor-addons' ), 'icon' => 'eicon-v-align-bottom' ],
				],
				'render_type' => 'template',
			]
		);
		...
		$document->start_controls_section(
			...
			[
				'label' => esc_html__( 'Pixels Popup Settings', 'pixels-elementor-addons' ),
				'tab'   => \Elementor\Controls_Manager::TAB_CONTENT,
			]
		);
		...
		$document->add_control(
			...
			[
				'label'        => esc_html__( 'Overlay', 'pixels-elementor-addons' ),
				'type'         => \Elementor\Controls_Manager::SWITCHER,
		...
		$document->add_control(
			...
			[
				'label'       => esc_html__( 'Overlay Color', 'pixels-elementor-addons' ),
		...
		$document->add_control(
			...
			[
				'label'        => esc_html__( 'Close Button', 'pixels-elementor-addons' ),
				'type'         => \Elementor\Controls_Manager::SWITCHER,
				'label_on'     => esc_html__( 'Show', 'pixels-elementor-addons' ),
				'label_off'    => esc_html__( 'Hide', 'pixels-elementor-addons' ),
				'return_value' => 'yes',
		...
		foreach ( [ 'entrance' => esc_html__( 'Entrance Animation', 'pixels-elementor-addons' ), 'exit' => esc_html__( 'Exit Animation', 'pixels-elementor-addons' ) ] as $key => $label ) {
			$document->add_control(
		...
		$document->add_control(
			...
			[
				'label'   => esc_html__( 'Automatically Close After (sec)', 'pixels-elementor-addons' ),
		...
		$document->start_controls_section(
			...
			[
				'label' => esc_html__( 'Pixels Popup Close Button', 'pixels-elementor-addons' ),
				'tab'   => \Elementor\Controls_Manager::TAB_CONTENT,
			]
		);
		...
		$document->add_control(
			...
			[
				...
				'options'     => [
					'inside'  => esc_html__( 'Inside Dialog', 'pixels-elementor-addons' ),
					'outside' => esc_html__( 'Outside Dialog', 'pixels-elementor-addons' ),
				],
		...
		$document->add_control(
			...
			[
				'label'       => esc_html__( 'Show After (sec)', 'pixels-elementor-addons' ),
				'type'        => \Elementor\Controls_Manager::NUMBER,
				'min'         => 0,
				'step'        => 0.1,
		...
		foreach ( [
			'close_button_offset_x'      => esc_html__( 'Horizontal Offset', 'pixels-elementor-addons' ),
			'close_button_offset_y'      => esc_html__( 'Vertical Offset', 'pixels-elementor-addons' ),
			'close_button_size'          => esc_html__( 'Button Size', 'pixels-elementor-addons' ),
			'close_button_icon_size'     => esc_html__( 'Icon Size', 'pixels-elementor-addons' ),
			'close_button_border_width'  => esc_html__( 'Border Width', 'pixels-elementor-addons' ),
			'close_button_border_radius' => esc_html__( 'Border Radius', 'pixels-elementor-addons' ),
		] as $key => $label ) {
			$document->add_control(
				'pixels_popup_' . $key,
				[
		...
		foreach ( [
			'close_button_color'            => esc_html__( 'Icon Color', 'pixels-elementor-addons' ),
			'close_button_background_color' => esc_html__( 'Background Color', 'pixels-elementor-addons' ),
			'close_button_border_color'     => esc_html__( 'Border Color', 'pixels-elementor-addons' ),
		] as $key => $label ) {
			$document->add_control(
				'pixels_popup_' . $key,
				[
		...
		$document->start_controls_section(
			...
			[
				'label' => esc_html__( 'Pixels Popup Advanced', 'pixels-elementor-addons' ),
				'tab'   => \Elementor\Controls_Manager::TAB_CONTENT,
			]
		);

		foreach ( [
			'prevent_overlay_close' => esc_html__( 'Prevent Closing on Overlay', 'pixels-elementor-addons' ),
			'prevent_esc_close'     => esc_html__( 'Prevent Closing on ESC key', 'pixels-elementor-addons' ),
			'disable_scroll'        => esc_html__( 'Disable Page Scrolling', 'pixels-elementor-addons' ),
			'avoid_multiple'        => esc_html__( 'Avoid Multiple Popups', 'pixels-elementor-addons' ),
			'accessible_navigation' => esc_html__( 'Accessible Navigation', 'pixels-elementor-addons' ),
		] as $key => $label ) {
			$document->add_control(
				'pixels_popup_' . $key,
				[
		...
		$document->add_control(
			...
			[
				'label'       => esc_html__( 'Open By Selector', 'pixels-elementor-addons' ),
				'type'        => \Elementor\Controls_Manager::TEXT,
				'default'     => $options['open_selector'],
				'placeholder' => '#id, .class',
				'description' => esc_html__( 'Clicking this selector opens the popup.', 'pixels-elementor-addons' ),
				'render_type' => 'template',
			]
		);
		...
		$document->add_control(
			'pixels_popup_margin',
			[
				'label'      => esc_html__( 'Margin', 'pixels-elementor-addons' ),
		...
		$document->add_control(
			...
			[
				'label'      => esc_html__( 'Content Padding', 'pixels-elementor-addons' ),
				'type'       => \Elementor\Controls_Manager::DIMENSIONS,
				'size_units' => [ 'px', '%', 'em', 'rem' ],
		...
		foreach ( [
			'content_background_color' => esc_html__( 'Content Background Color', 'pixels-elementor-addons' ),
			'content_border_color'     => esc_html__( 'Content Border Color', 'pixels-elementor-addons' ),
		] as $key => $label ) {
			$document->add_control(
				'pixels_popup_' . $key,
				[
		...
		$document->add_control(
			...
			[
				'label'       => esc_html__( 'Content Border Style', 'pixels-elementor-addons' ),
				'type'        => \Elementor\Controls_Manager::SELECT,
				'default'     => $options['content_border_style'],
				'options'     => [
					'none'   => esc_html__( 'None', 'pixels-elementor-addons' ),
					'solid'  => esc_html__( 'Solid', 'pixels-elementor-addons' ),
					'dashed' => esc_html__( 'Dashed', 'pixels-elementor-addons' ),
					'dotted' => esc_html__( 'Dotted', 'pixels-elementor-addons' ),
					'double' => esc_html__( 'Double', 'pixels-elementor-addons' ),
				],
				'selectors'   => [
		...
		foreach ( [
			'content_border_width'  => esc_html__( 'Content Border Width', 'pixels-elementor-addons' ),
			'content_border_radius' => esc_html__( 'Content Border Radius', 'pixels-elementor-addons' ),
		] as $key => $label ) {
			$document->add_control(
				'pixels_popup_' . $key,
				[
		...
		$document->add_group_control(
			...
			[
				...
				'label'    => esc_html__( 'Content Box Shadow', 'pixels-elementor-addons' ),
		...
		$document->add_control(
			...
			[
				'label'   => esc_html__( 'CSS Classes', 'pixels-elementor-addons' ),
				'type'    => \Elementor\Controls_Manager::TEXT,
	...
	public function register_mega_menu_document_controls( $document ) {
		...
		if ( ! $post || 'pixels-theme' !== $post->post_type || 'type_mega_menu' !== get_post_meta( $post->ID, 'ehf_template_type', true ) ) {
			return;
		}
		...
		$document->start_controls_section(
			...
			[
				'label' => esc_html__( 'Pixels Mega Menu Layout', 'pixels-elementor-addons' ),
				'tab'   => \Elementor\Controls_Manager::TAB_CONTENT,
			]
		);
		...
		$document->add_control(
			...
			[
				'label'   => esc_html__( 'Width Type', 'pixels-elementor-addons' ),
				'type'    => \Elementor\Controls_Manager::SELECT,
				'default' => $options['width_type'],
				'options' => [
					'custom'     => esc_html__( 'Custom Width', 'pixels-elementor-addons' ),
					'container'  => esc_html__( 'Container Width', 'pixels-elementor-addons' ),
					'full_width' => esc_html__( 'Full Screen Width', 'pixels-elementor-addons' ),
				],
			]
		);
		...
		$document->add_responsive_control(
			...
			[
				'label'      => esc_html__( 'Container Width', 'pixels-elementor-addons' ),
				'type'       => \Elementor\Controls_Manager::SLIDER,
				'size_units' => [ 'px', 'vw' ],
				'range'      => [
					'px' => [ 'min' => 320, 'max' => 2000 ],
	...
	public function preserve_popup_position_settings_on_save( $data, $document ) {
		...
		if ( ! $post_id || 'pixels-theme' !== get_post_type( $post_id ) || 'type_popup' !== get_post_meta( $post_id, 'ehf_template_type', true ) ) {
			return $data;
		}
	...
	public function register_single_post_preview_controls( $document ) {
		...
		if ( ! $post || 'pixels-theme' !== $post->post_type || 'type_single_post' !== get_post_meta( $post->ID, 'ehf_template_type', true ) ) {
			return;
		}
		...
		$document->start_controls_section(
			...
			[
				'label' => esc_html__( 'Preview', 'pixels-elementor-addons' ),
		...
		$document->add_control(
			...
			[
				'label'       => esc_html__( 'Preview with Post', 'pixels-elementor-addons' ),
				'type'        => \Elementor\Controls_Manager::SELECT,
				'default'     => $preview_post_id ? (string) $preview_post_id : '',
				'options'     => pixels_get_theme_builder_preview_post_select_options(),
				'description' => esc_html__( 'Choose a post to preview dynamic widgets. The editor preview stays on this template.', 'pixels-elementor-addons' ),
			]
		);
	...
	public function enqueue_scripts() {
		...
		if ( is_singular( 'pixels-theme' ) && 'type_popup' === get_post_meta( get_the_ID(), 'ehf_template_type', true ) ) :
	...
	public function enqueue_single_post_preview_editor_script() {
		...
		if ( ! $post_id || 'pixels-theme' !== get_post_type( $post_id ) || 'type_single_post' !== get_post_meta( $post_id, 'ehf_template_type', true ) ) {
			return;
		}
		...
		return array_map(
			...
			get_posts(
				[
					...
					'posts_per_page' => -1,
					'fields'         => 'ids',
					'meta_key'       => 'ehf_template_type', // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_key
	...
	private function get_mega_menu_template_id_from_item( $item ) {
		...
		if ( ! $template_id || 'type_mega_menu' !== get_post_meta( $template_id, 'ehf_template_type', true ) ) :
			return 0;
		endif;
	...
	public function render_popups() {
		...
		foreach ( $popup_ids as $popup_id ) :
			...
			?>
			<?php echo $this->get_popup_responsive_css( '#pixels-theme-popup-' . absint( $popup_id ), $popup_id, $options ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
			<div
			...
			>
				...
				<div class="pixels-theme-popup__dialog" role="dialog" aria-modal="true" tabindex="-1" style="<?php echo esc_attr( $dialog_style ); ?>">
					<?php if ( 'yes' === $options['show_close_button'] ) : ?>
						<button type="button" class="pixels-theme-popup__close" data-pixels-popup-close aria-label="<?php esc_attr_e( 'Close popup', 'pixels-elementor-addons' ); ?>" style="<?php echo esc_attr( $close_button_style ); ?>">
	...
	public function render_popup_editor_template() {
		...
		$close_button_style = $this->get_popup_close_button_style( $options );
		?>
		<?php echo $this->get_popup_responsive_css( '#pixels-theme-popup-editor-preview-' . absint( $popup_id ), $popup_id, $options ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
		<div id="pixels-theme-popup-editor-preview-<?php echo esc_attr( (string) $popup_id ); ?>" class="<?php echo esc_attr( implode( ' ', $classes ) ); ?>" aria-hidden="false">
			...
			<div class="pixels-theme-popup__dialog" role="dialog" aria-modal="true" tabindex="-1" style="<?php echo esc_attr( $dialog_style ); ?>">
				<button type="button" class="pixels-theme-popup__close" aria-label="<?php esc_attr_e( 'Close popup', 'pixels-elementor-addons' ); ?>" style="<?php echo esc_attr( $close_button_style . ( 'yes' === $options['show_close_button'] ? '' : 'display:none;' ) ); ?>">
					<span aria-hidden="true"></span>
				</button>
				<div class="pixels-theme-popup__content" style="<?php echo esc_attr( $content_style ); ?>">
					<?php
	...
	public function open_popup_editor_wrapper() {
		...
		if ( ! $popup_id || 'pixels-theme' !== get_post_type( $popup_id ) || 'type_popup' !== get_post_meta( $popup_id, 'ehf_template_type', true ) ) :
			return;
		endif;
	...
	private function get_popup_responsive_css( $selector, $popup_id, $options ) {
		$page_settings = get_post_meta( $popup_id, '_elementor_page_settings', true );
		$page_settings = is_array( $page_settings ) ? $page_settings : [];
		...
		$css                = sprintf(
			'<style>%1$s{justify-content:%2$s;align-items:%3$s;}%1$s .pixels-theme-popup__dialog{--pixels-popup-width:%4$s;}',
			esc_html( $selector ),
			esc_html( $desktop_horizontal ),
			esc_html( $desktop_vertical ),
			esc_html( $desktop_width )
		);
		...
		foreach ( $responsive_devices as $device => $breakpoint ) {
			...
			if ( $device_horizontal ) {
				$device_css .= sprintf( 'justify-content:%s;', esc_html( $device_horizontal ) );
			}

			if ( $device_vertical ) {
				$device_css .= sprintf( 'align-items:%s;', esc_html( $device_vertical ) );
			}

			if ( $device_css ) {
				$css .= sprintf( '@media (max-width:%1$s){%2$s{%3$s}}', esc_html( $breakpoint ), esc_html( $selector ), $device_css );
			}
			...
			if ( $device_width ) {
				$css .= sprintf(
					...
					esc_html( $breakpoint ),
					esc_html( $selector ),
					esc_html( $this->sanitize_popup_css_size( $device_width, $desktop_width ) )
				);
			}
		}
	...
	private function get_popup_trigger_options() {
		return [
			'page_load'   => [
				'label'    => esc_html__( 'On Page Load', 'pixels-elementor-addons' ),
				'controls' => [
					'page_load_delay' => [
						'label' => esc_html__( 'Within (sec)', 'pixels-elementor-addons' ),
						'type'  => \Elementor\Controls_Manager::NUMBER,
						'min'   => 0,
						'step'  => 0.1,
					],
			...
			'scroll'      => [
				'label'    => esc_html__( 'On Scroll', 'pixels-elementor-addons' ),
				'controls' => [
					'scroll_direction' => [
						'label'   => esc_html__( 'Direction', 'pixels-elementor-addons' ),
						'type'    => \Elementor\Controls_Manager::SELECT,
						'options' => [
							'down' => esc_html__( 'Down', 'pixels-elementor-addons' ),
							'up'   => esc_html__( 'Up', 'pixels-elementor-addons' ),
						],
					],
					'scroll_percent'   => [
						'label' => esc_html__( 'Within (%)', 'pixels-elementor-addons' ),
						'type'  => \Elementor\Controls_Manager::NUMBER,
						'min'   => 0,
						'max'   => 100,
						'step'  => 1,
			...
			'scroll_to'   => [
				'label'    => esc_html__( 'On Scroll To Element', 'pixels-elementor-addons' ),
				'controls' => [
					'scroll_to_selector' => [
						'label'       => esc_html__( 'Selector', 'pixels-elementor-addons' ),
						'type'        => \Elementor\Controls_Manager::TEXT,
			...
			'class_click' => [
				'label'    => esc_html__( 'On Class Click', 'pixels-elementor-addons' ),
				'controls' => [
					'class_click_selector' => [
						'label'       => esc_html__( 'Class / Selector', 'pixels-elementor-addons' ),
						'type'        => \Elementor\Controls_Manager::TEXT,
						'placeholder' => '.my-class',
					],
				],
			...
			'click'       => [
				'label'    => esc_html__( 'On Click', 'pixels-elementor-addons' ),
				'controls' => [
					'click_count' => [
						'label' => esc_html__( 'Clicks', 'pixels-elementor-addons' ),
						'type'  => \Elementor\Controls_Manager::NUMBER,
						'min'   => 1,
						'step'  => 1,
					],
			...
			'inactivity'  => [
				'label'    => esc_html__( 'After Inactivity', 'pixels-elementor-addons' ),
				'controls' => [
					'inactivity_delay' => [
						'label' => esc_html__( 'Within (sec)', 'pixels-elementor-addons' ),
						'type'  => \Elementor\Controls_Manager::NUMBER,
			...
			'exit_intent' => [
				'label'    => esc_html__( 'On Page Exit Intent', 'pixels-elementor-addons' ),
				'controls' => [],
			],
			'adblock'     => [
				'label'    => esc_html__( 'AdBlock Detection', 'pixels-elementor-addons' ),
				'controls' => [
					'adblock_delay' => [
						'label' => esc_html__( 'Within (sec)', 'pixels-elementor-addons' ),
						'type'  => \Elementor\Controls_Manager::NUMBER,
	...
	private function get_popup_animation_options() {
		return [
			'default'     => esc_html__( 'Default', 'pixels-elementor-addons' ),
			'fade'        => esc_html__( 'Fade', 'pixels-elementor-addons' ),
			'zoom'        => esc_html__( 'Zoom', 'pixels-elementor-addons' ),
			'slide-up'    => esc_html__( 'Slide Up', 'pixels-elementor-addons' ),
			'slide-down'  => esc_html__( 'Slide Down', 'pixels-elementor-addons' ),
			'slide-left'  => esc_html__( 'Slide Left', 'pixels-elementor-addons' ),
			'slide-right' => esc_html__( 'Slide Right', 'pixels-elementor-addons' ),
		];
	}
	...
	public function render_template( $atts ) {
		...
		$id = ! empty( $atts['id'] ) ? apply_filters( 'pixels_hf_render_template_id', intval( $atts['id'] ) ) : '';
	...
	public function maybe_use_popup_canvas_template( $template ) {
		...
		if ( ! $post_id || 'type_popup' !== get_post_meta( $post_id, 'ehf_template_type', true ) ) {
			return $template;
		}
