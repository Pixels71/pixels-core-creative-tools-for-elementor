...
class Admin {
	...
	public function column_headings( $columns ) {
		...
		$columns['pixels_hf_display_rules'] = esc_html__( 'Display Rules', 'pixels-elementor-addons' );
		$columns['date']                  = esc_html__( 'Date', 'pixels-elementor-addons' );
	...
	public function column_content( $column, $post_id ) {
		...
		if ( 'pixels_hf_display_rules' == $column ) :
			...
			if ( ! empty( $locations ) ) :
				...
				echo '<strong>' . esc_html__( 'Display:', 'pixels-elementor-addons' ) . ' </strong>';
			...
			if ( ! empty( $locations ) ) :
				echo '<div class="pixels-theme-advanced-headers-exclusion-wrap" style="margin-bottom: 5px;">';
					echo '<strong>' . esc_html__( 'Exclusion:', 'pixels-elementor-addons' ) . ' </strong>';
					$this->column_display_location_rules( $locations );
				echo '</div>';
			endif;
		endif;
	...
	public function header_footer_posttype() {
		$labels = [
			'name'               => esc_html__( 'Theme Builder', 'pixels-elementor-addons' ),
			'singular_name'      => esc_html__( 'Theme Builder', 'pixels-elementor-addons' ),
			'menu_name'          => esc_html__( 'Theme Builder', 'pixels-elementor-addons' ),
			'name_admin_bar'     => esc_html__( 'Theme Builder', 'pixels-elementor-addons' ),
			'add_new'            => esc_html__( 'Add New', 'pixels-elementor-addons' ),
			'add_new_item'       => esc_html__( 'Add New Template', 'pixels-elementor-addons' ),
			'new_item'           => esc_html__( 'New Template', 'pixels-elementor-addons' ),
			'edit_item'          => esc_html__( 'Edit Template', 'pixels-elementor-addons' ),
			'view_item'          => esc_html__( 'View Template', 'pixels-elementor-addons' ),
			'all_items'          => esc_html__( 'All Templates', 'pixels-elementor-addons' ),
			'search_items'       => esc_html__( 'Search Templates', 'pixels-elementor-addons' ),
			'parent_item_colon'  => esc_html__( 'Parent Template', 'pixels-elementor-addons' ),
			'not_found'          => esc_html__( 'No Templates found.', 'pixels-elementor-addons' ),
			'not_found_in_trash' => esc_html__( 'No Templates found in Trash.', 'pixels-elementor-addons' )
		];
	...
	public function register_admin_menu() {
		add_menu_page(
			esc_html__( 'Theme Builder', 'pixels-elementor-addons' ), // Page title
			esc_html__( 'Theme Builder', 'pixels-elementor-addons' ), // Menu title
			'edit_pages', // Capability
			'edit.php?post_type=pixels-theme', // Menu slug (links directly to your CPT)
			'', // Callback not needed since it redirects to post type
			'dashicons-editor-kitchensink', // Icon (change as you like)
			21 // Position in the admin menu
		);
	}
	...
	public function register_mega_menu_nav_metabox() {
		add_meta_box(
			...
			esc_html__( 'Mega Menu', 'pixels-elementor-addons' ),
	...
	public function render_mega_menu_nav_metabox() {
		...
		if ( empty( $mega_menus ) ) :
			?>
			<p><?php esc_html_e( 'No published Mega Menu templates found.', 'pixels-elementor-addons' ); ?></p>
			<p>
				<a href="<?php echo esc_url( admin_url( 'post-new.php?post_type=pixels-theme' ) ); ?>">
					<?php esc_html_e( 'Create a Mega Menu template', 'pixels-elementor-addons' ); ?>
				</a>
			</p>
			<?php
			return;
		...
		<div id="pixels-mega-menu" class="posttypediv">
			...
			<p class="button-controls wp-clearfix">
				<span class="list-controls hide-if-no-js">
					...
					<label for="pixels-mega-menu-tab"><?php esc_html_e( 'Select All', 'pixels-elementor-addons' ); ?></label>
				</span>
				<span class="add-to-menu">
					<input type="submit" class="button-secondary submit-add-to-menu right" value="<?php esc_attr_e( 'Add to Menu', 'pixels-elementor-addons' ); ?>" name="add-pixels-mega-menu-menu-item" id="submit-pixels-mega-menu">
	...
	function register_metabox() {
		add_meta_box(
			...
			__( 'Pixels Theme Builder', 'pixels-elementor-addons' ),
			[
				$this,
				'metabox_render',
			],
			...
			<tbody>
				<tr class="pixels-theme-options-row type-of-template">
					<td class="pixels-theme-options-row-heading">
						<label for="ehf_template_type"><?php esc_html_e( 'Type of Template', 'pixels-elementor-addons' ); ?></label>
					</td>
					<td class="pixels-theme-options-row-content">
						<select name="ehf_template_type" id="ehf_template_type">
							<option value="" <?php selected( $template_type, '' ); ?>><?php esc_html_e( 'Select Option', 'pixels-elementor-addons' ); ?></option>
							<?php foreach ( \PixelsElementorAddons\Plugin::get_theme_template_types() as $type_key => $type_label ) : ?>
	...
	public function display_rules_tab() {
		...
		<tr class="pixels-theme-target-rules-row pixels-theme-options-row">
			<td class="pixels-theme-target-rules-row-heading pixels-theme-options-row-heading">
				<label><?php esc_html_e( 'Display On', 'pixels-elementor-addons' ); ?></label>
				<i class="pixels-theme-target-rules-heading-help dashicons dashicons-editor-help"
					title="<?php echo esc_attr__( 'Add locations for where this template should appear.', 'pixels-elementor-addons' ); ?>"></i>
			</td>
			<td class="pixels-theme-target-rules-row-content pixels-theme-options-row-content">
				...
				Target_Rules_Fields::target_rule_settings_field(
					...
					[
						'title'          => esc_html__( 'Display Rules', 'pixels-elementor-addons' ),
						'value'          => '[{"type":"basic-global","specific":null}]',
						'tags'           => 'site,enable,target,pages',
						'rule_type'      => 'display',
						'add_rule_label' => esc_html__( 'Add Display Rule', 'pixels-elementor-addons' ),
					],
					$include_locations
				);
				?>
		...
		<tr class="pixels-theme-target-rules-row pixels-theme-options-row">
			<td class="pixels-theme-target-rules-row-heading pixels-theme-options-row-heading">
				<label><?php esc_html_e( 'Do Not Display On', 'pixels-elementor-addons' ); ?></label>
				<i class="pixels-theme-target-rules-heading-help dashicons dashicons-editor-help"
					title="<?php echo esc_attr__( 'Add locations for where this template should not appear.', 'pixels-elementor-addons' ); ?>"></i>
			</td>
			<td class="pixels-theme-target-rules-row-content pixels-theme-options-row-content">
				<?php
				Target_Rules_Fields::target_rule_settings_field(
					...
					[
						'title'          => esc_html__( 'Exclude On', 'pixels-elementor-addons' ),
						'value'          => '[]',
						'tags'           => 'site,enable,target,pages',
						'add_rule_label' => esc_html__( 'Add Exclusion Rule', 'pixels-elementor-addons' ),
						'rule_type'      => 'exclude'
					],
					$exclude_locations
				);
	...
	public function save_meta( $post_id ) {
		...
		if ( isset( $_POST['ehf_template_type'] ) ) {
			$ehf_template_type = sanitize_text_field( wp_unslash( $_POST['ehf_template_type'] ) );
			$allowed_types    = array_keys( \PixelsElementorAddons\Plugin::get_theme_template_types() );

			if ( '' !== $ehf_template_type && ! in_array( $ehf_template_type, $allowed_types, true ) ) {
				$ehf_template_type = '';
			}

			update_post_meta( $post_id, 'ehf_template_type', $ehf_template_type );

			if ( 'type_popup' === $ehf_template_type ) {
	...
	public function location_notice() {
		...
		if ( '' !== $template_type ) :
			$templates = \PixelsElementorAddons\Theme_Builder\Theme_Elementor::get_template_id( $template_type );
			...
			if ( is_array( $templates ) && isset( $templates[1] ) && $post->ID != $templates[0] ) :
				...
				$message = sprintf(
					...
					__( 'Template "%1$s" is already assigned to the location "%2$s".', 'pixels-elementor-addons' ),
					$post_title,
					$template_location
				);
	...
	public function template_location( $template_type ) {
		$labels = [
			'type_header'       => __( 'Header', 'pixels-elementor-addons' ),
			'type_footer'       => __( 'Footer', 'pixels-elementor-addons' ),
			'type_archive_post' => __( 'Archive Post', 'pixels-elementor-addons' ),
			'type_404'          => __( '404', 'pixels-elementor-addons' ),
			'type_single_post'  => __( 'Single Post', 'pixels-elementor-addons' ),
			'type_popup'        => __( 'Popup', 'pixels-elementor-addons' ),
			'type_mega_menu'    => __( 'Mega Menu', 'pixels-elementor-addons' ),
		];
	...
	function set_shortcode_columns( $columns ) {
		...
		$columns['shortcode'] = esc_html__( 'Shortcode', 'pixels-elementor-addons' );
		$columns['type'] = esc_html__( 'Type', 'pixels-elementor-addons' );
	...
	public function pixels_hf_filter_post_type_by_meta( $query ) {
		...
		if ( is_admin() && 'edit.php' === $pagenow && 'pixels-theme' === $post_type && ! empty( $meta_value ) ) {
			$query->query_vars['meta_key']   = 'ehf_template_type'; // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_key
	...
	public function pixels_admin_theme_builder_tabs( $views ) {
		...
		if ( 'pixels-theme' === $typenow ) {
			...
			$types       = \PixelsElementorAddons\Plugin::get_theme_template_types();
			?>
			<div class="nav-tab-wrapper">
				<a href="<?php echo esc_url( admin_url( 'edit.php?post_type=pixels-theme' ) ); ?>" class="nav-tab <?php echo 'all' === $current_tab ? 'nav-tab-active' : ''; ?>" data-template-type=""><?php esc_html_e( 'All', 'pixels-elementor-addons' ); ?></a>
	...
	public function register_preview_metabox() {
		...
		add_meta_box(
			...
			__( 'Preview Settings', 'pixels-elementor-addons' ),
	...
	public function render_preview_metabox( $post ) {
		...
		<p>
			...
			if ( 'type_single_post' === $template_type ) {
				esc_html_e( 'Choose a post to preview dynamic widgets and the Elementor live preview.', 'pixels-elementor-addons' );
			} else {
				esc_html_e( 'Choose a post to determine which archive page is used for preview.', 'pixels-elementor-addons' );
			}
			?>
		</p>
		<p><label for="pixels_preview_post_id"><?php esc_html_e( 'Preview with post:', 'pixels-elementor-addons' ); ?></label></p>
		<select name="pixels_preview_post_id" id="pixels_preview_post_id" class="widefat">
			<option value=""><?php esc_html_e( 'Select a post', 'pixels-elementor-addons' ); ?></option>
			<?php
