...
if ( ! class_exists( 'Update' ) ) :
	...
	class Update {
		 ...
		 * @var string
		 */
		private $db_option_key = '_pixels_core_hf_db_version';
		...
		public function init() {
			do_action( 'pixels_core_hf_update_before' );
			...
			do_action( 'pixels_core_hf_update_after' );
		}
		...
		private function setup_default_terget_rules() {
			...
			// Header.
			if ( ! empty( $header_id ) ) :
				update_post_meta( $header_id, 'pixels_core_hf_target_include_locations', $default_include_locations );
			endif;

			// Footer.
			if ( ! empty( $footer_id ) ) :
				update_post_meta( $footer_id, 'pixels_core_hf_target_include_locations', $default_include_locations );
			endif;
		}
		...
		public function get_legacy_template_id( $type ) {
			...
			$cache_key      = 'pixels_core_legacy_template_' . md5( $type_sanitized );
			...
			$args = [
				...
				'meta_query'     => [ // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_query
					[
						'key'     => pixels_core_get_theme_template_type_meta_key(),
			...
			$args     = apply_filters( 'pixels_core_hf_get_template_id_args', $args );
