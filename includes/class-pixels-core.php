...
use PixelsElementorAddons\Admin\Admin_Loader;
use PixelsElementorAddons\Admin\Extension_Settings;
...
final class Plugin {
	...
	public function init(): void {
		...
		/**
		 * Fires after Pixels Addons for Elementor has initialized Elementor integrations.
		 * Used by Pixels Addons Pro to boot form handlers and related features.
		 */
	...
	public function admin_notice_missing_elementor(): void {
		...
		$message = sprintf(
			...
			esc_html__( '%1$s requires %2$s to be installed and activated.', 'pixels-elementor-addons' ),
			'<strong>' . esc_html__( 'Pixels Addons for Elementor', 'pixels-elementor-addons' ) . '</strong>',
			'<strong>' . esc_html__( 'Elementor', 'pixels-elementor-addons' ) . '</strong>'
		);
	...
	public function admin_notice_minimum_elementor_version(): void {
		...
		$message = sprintf(
			...
			esc_html__( '%1$s requires %2$s version %3$s or greater.', 'pixels-elementor-addons' ),
			'<strong>' . esc_html__( 'Pixels Addons for Elementor', 'pixels-elementor-addons' ) . '</strong>',
			'<strong>' . esc_html__( 'Elementor', 'pixels-elementor-addons' ) . '</strong>',
			'3.5.0'
		);
	...
	public function admin_notice_missing_nested_elements(): void {
		...
		$message = sprintf(
			...
			esc_html__( '%1$s nested widgets require the %2$s experiment to be enabled in Elementor → Settings → Features.', 'pixels-elementor-addons' ),
			'<strong>' . esc_html__( 'Pixels Addons for Elementor', 'pixels-elementor-addons' ) . '</strong>',
			'<strong>' . esc_html__( 'Nested Elements', 'pixels-elementor-addons' ) . '</strong>'
		);
